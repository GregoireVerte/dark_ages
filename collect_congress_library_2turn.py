...
MAX_PAGES_PER_KEYWORD = 75      ## max stron na jedno hasło
...
for kw in keywords:
    ...
    while page <= MAX_PAGES_PER_KEYWORD:
        ...
        time.sleep(10)  ### rate limiting – b. ważne!!

    time.sleep(20)  ## przerwa między keywordami

# zapisz do csv
with open("congress_articles_2turn.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=["keyword"] + desired_fields + ["item_url"])
    writer.writeheader()
    writer.writerows(all_results)

print(f"\nZebrano łącznie {len(all_results)} rekordów. Zapisano do congress_articles_2turn.csv")